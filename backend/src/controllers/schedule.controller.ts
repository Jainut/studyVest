...
export async function createSchedule(req: Request, res: Response) {
  ...
  if (data.topicId) {
    ...
    if (!topic) {
      throw new AppError('Conteúdo não encontrado nesta matéria.', 404);
    }
  }
...
export async function updateSchedule(req: Request, res: Response) {
  ...
  const existing = await prisma.schedule.findFirst({ where: { id, userId: req.userId } });
  if (!existing) {
  ...
  if (data.topicId) {
    const topic = await prisma.topic.findFirst({
      where: { id: data.topicId, subjectId: existing.subjectId, userId: req.userId },
    });
    if (!topic) {
      throw new AppError('Conteúdo não encontrado nesta matéria.', 404);
    }
